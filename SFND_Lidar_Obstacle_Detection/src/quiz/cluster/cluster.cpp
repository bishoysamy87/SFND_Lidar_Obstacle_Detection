			   ...
			   std::vector<int>& cluster)
{
	...
	for(int index : nearby)
	{
		...
		if(is_processed[index] == 0)
		{
			...
			Proximity(points, is_processed,tree ,distanceTol,index,cluster);
		}
	}
	//std::cout << std::endl;
}
...
std::vector<std::vector<int>> euclideanCluster(const std::vector<std::vector<float>>& points, KdTree* tree, float distanceTol)
{
	...
	is_processed.assign(points.size(),0);
	...
	for (int itr = 0; itr < points.size(); itr++)
	{
		if(is_processed[itr] == 0)
		{
			...
			std::vector<int> cluster;
			Proximity(points,is_processed,tree,distanceTol,itr,cluster);
